...
public class MainCharacterState : MonoBehaviour, NpcHelper
{
    ...
    public void AnimationAttack(int attackType)//事件觸發
    {
        ...
        if (attackType == 4)//終結技
            NpcCommon.AttackDetection("Pico", transform.position, transform.forward, 360, 4f, true, new DamageData(200f, transform.forward * 0.15f, HitType.Heavy, new DamageStateInfo(DamageState.Finishing, 0)), "Npc");
        if (attackType == 5)//無雙
